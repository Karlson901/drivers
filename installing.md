Ниже — установка **полностью из уже скачанного** `Xerox_Phaser_3020_Linux-Driver.tar.gz`, без интернета. В источниках для Phaser 3020 указано, что архив распаковывается в каталог `uld`, а затем запускается `install-printer.sh` или `install.sh`.[^1][^2]
